...
class DataPath():
    ...
    def alu_op(self, sel_instr: bool, second_operand: int | None = None, opcode: Opcode | None = None):
        alu_out = self.acc
        if not sel_instr: second_operand = self.memory_output
        
        if opcode is Opcode.LD: alu_out = second_operand
        if opcode is Opcode.ST: alu_out = second_operand

        if opcode is Opcode.ADD: alu_out += second_operand
        if opcode is Opcode.SUB: alu_out -= second_operand
        if opcode is Opcode.CMP:
            alu_out -= second_operand
            self.flag_z = alu_out == 0
            self.flag_n = alu_out < 0
        if opcode is Opcode.MUL: alu_out *= second_operand
        if opcode is Opcode.DIV: alu_out //= second_operand
        if opcode is Opcode.MOD: alu_out %= second_operand
...
class ControlUnit():
    ...
    def decode_and_execute_control_flow_instruction(self, instr: Instruction) -> bool:
        ...
        if instr.opcode is Opcode.JMP: sel_next = False
    ...
    def __repr__(self):
        return (
            ...
            f'MEM_OUT: {self.data_path.memory_output}    '
            f'ALU: {self.data_path.alu}    '
